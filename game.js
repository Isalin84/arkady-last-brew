...
let map,props=[],levelIndex=0,levelTotal=12,transition=false,checkpoint=null,pathField=[],pathAt=-1;
...
function loadLevel(index,restore=false){
 GameAudio.reset();pathAt=-1;pathField=[];levelIndex=index;const level=LEVELS[index];map=level.map.map(r=>r.split('').map(Number));
 ...
 enemies=level.enemies.map(([x,y,type],i)=>({x,y,type,hp:ENEMY_TYPES[type].hp,max:ENEMY_TYPES[type].hp,attack:0,hit:0,seed:i*3.1,alert:false}));levelTotal=enemies.length;
...
function start(){
 ...
 if(transition){checkpoint={hp:Math.max(75,player.hp),ammo:weapons.map((w,i)=>i===0?Infinity:Math.max(w.ammo,[0,22,140,100][i]))};loadLevel(levelIndex+1,true);event='packstart';}
 else if(dead){loadLevel(levelIndex,true);event=levelIndex?'packstart':'start';}
 else if(!started||won)reset();
 started=true;running=true;$('#overlay').hidden=true;$('#hud').hidden=false;$('#crosshair').style.display='block';$('#status').textContent='Очисти цех и доберись до выхода';
...
function pause(){if(!running)return;GameAudio.pause();running=false;fire=false;keys.clear();$('#overlay').hidden=false;$('.intro h1').innerHTML='ПЕРЕРЫВ<br><span>НА ПЕНУ</span>';$('.intro p').innerHTML='Смена ещё не закончена.<br>Цех ждёт своего пивовара.';$('#start').innerHTML='Продолжить смену <span>↗</span>';document.exitPointerLock?.();}
function finish(win){
 GameAudio.pause();transition=win&&levelIndex<LEVELS.length-1;GameAudio.say(transition?'transition':win?'finalwin':'death',true);
 running=false;won=win&&!transition;dead=!win;fire=false;keys.clear();document.exitPointerLock?.();$('#overlay').hidden=false;
 $('.intro h1').innerHTML=transition?'ДАЛЬШЕ —<br><span>РОЗЛИВ</span>':win?'ПАРТИЯ<br><span>СПАСЕНА</span>':'СМЕНА<br><span>ПРОПАЛА</span>';
 $('.intro p').innerHTML=transition?'Варочный цех очищен. Но тара ожила!<br>Впереди линии банок и бутылок, 16 новых монстров.<br>Припасы пополнены, здоровье — не ниже 75.':win?'Оба цеха очищены. Все 28 монстров уничтожены.<br>Пивоварня снова работает. Аркадий заслужил выходной.':`Уничтожено: ${kills} из ${levelTotal}.<br>Попробуй ещё раз с начала этого цеха.`;
 $('#start').innerHTML=(transition?'В цех упаковки':win?'Новая смена':'Повторить цех')+' <span>↗</span>';
 $('#status').textContent=transition?'Уровень 1 из 2 пройден':win?'Два цеха спасены':'Смена прервана';
}
function shoot(){let w=weapons[weapon];if(cooldown>0)return;if(w.ammo<=0){toast('Припасы закончились. Найди ящик или возьми бутылку: 1');cooldown=.4;return;}w.ammo--;cooldown=w.cool;kick=1;let count=weapon===3?3:1;for(let i=0;i<count;i++){let a=player.a+(i-(count-1)/2)*.1;shots.push({x:player.x+Math.cos(a)*.25,y:player.y+Math.sin(a)*.25,dx:Math.cos(a)*w.speed,dy:Math.sin(a)*w.speed,type:w.type,damage:w.damage,life:weapon===3?.65:2.5,age:0});}GameAudio.shot(w.type);GameAudio.say(levelIndex?'packshoot':'shoot');updateHUD();}
...
function damage(e,d){if(e.hp<=0)return;e.hp-=d;e.hit=.16;if(e.hp<=0){kills++;burst(e.x,e.y,ENEMY_TYPES[e.type].color,25);GameAudio.kill();GameAudio.say(e.type===5?'golemkill':e.type===3?'packkill':'kill');if(kills===levelTotal)toast('Цех очищен! Выход — в дальнем правом углу. Смотри карту: M');updateHUD();}}
...
function tick(dt){clock+=dt;cooldown=Math.max(0,cooldown-dt);kick=Math.max(0,kick-dt*4);hurt=Math.max(0,hurt-dt*2);toastTime-=dt;if(toastTime<=0)$('#toast').style.opacity=0;if(!running)return;
...
for(let e of enemies){if(e.hp<=0)continue;e.hit=Math.max(0,e.hit-dt);e.attack-=dt;let d=Math.hypot(e.x-player.x,e.y-player.y),visible=sight(e.x,e.y,player.x,player.y);if((d<8&&visible)||e.alert){if(!e.alert){e.alert=true;toast(ENEMY_TYPES[e.type].name+'!');GameAudio.monster?.(e.type,d,(-Math.sin(player.a)*(e.x-player.x)+Math.cos(player.a)*(e.y-player.y))/5);}if(d>.72||!visible){chase(e,dt,d,visible);}else if(e.attack<=0){player.hp=Math.max(0,player.hp-ENEMY_TYPES[e.type].damage);hurt=1;e.attack=e.type===5?1.4:.85;GameAudio.monster?.(e.type,d,0,true);GameAudio.hit();GameAudio.say(player.hp<30?'low':levelIndex?'packhurt':'hurt');updateHUD();if(player.hp<=0){finish(false);return;}}}}
for(let sh of shots){sh.life-=dt;sh.age+=dt;let n=Math.ceil(Math.hypot(sh.dx,sh.dy)*dt/.1);for(let j=0;j<n&&sh.life>0;j++){sh.x+=sh.dx*dt/n;sh.y+=sh.dy*dt/n;if(solid(sh.x,sh.y)){sh.x-=sh.dx*dt/n;sh.y-=sh.dy*dt/n;impact(sh);sh.life=0;break;}let target=enemies.find(e=>e.hp>0&&Math.hypot(e.x-sh.x,e.y-sh.y)<.34);if(target){if(sh.type!=='can')damage(target,sh.damage);impact(sh);sh.life=0;}}if(sh.life<=0&&sh.type==='can'&&sh.age>=2.5)impact(sh);}
...
studioLogo.onload=()=>{const size=82;const scale=Math.min(size/studioLogo.naturalWidth,size/studioLogo.naturalHeight);const w=studioLogo.naturalWidth*scale*(2*VIEW*Math.tan(FOV/2)/W),h=studioLogo.naturalHeight*scale;for(const wall of [studioWall,studioPackWall])wall.getContext('2d').drawImage(studioLogo,64-w/2,64-h/2,w,h);};
...
function background(){ctx.fillStyle='#263629';ctx.fillRect(0,0,W,H);let sky=ctx.createLinearGradient(0,0,0,VIEW/2);sky.addColorStop(0,LEVELS[levelIndex].ceiling);sky.addColorStop(1,levelIndex?'#72909b':'#68745e');ctx.fillStyle=sky;ctx.fillRect(0,0,W,VIEW/2);let floor=ctx.createLinearGradient(0,VIEW/2,0,VIEW);floor.addColorStop(0,LEVELS[levelIndex].floor);floor.addColorStop(1,'#302f24');ctx.fillStyle=floor;ctx.fillRect(0,VIEW/2,W,H);for(let y=VIEW/2+5;y<VIEW;y+=3){let d=VIEW/(2*(y-VIEW/2));let alpha=Math.min(.17,.8/d);ctx.fillStyle=`rgba(195,171,109,${alpha})`;if(Math.floor(d+player.y)%2===0)ctx.fillRect(0,y,W,1);} }
function floorDetails(){
 ...
 const fx=wx-Math.floor(wx),fy=wy-Math.floor(wy);if(fx<.035||fy<.035){ctx.fillStyle=levelIndex?`rgba(15,39,50,${alpha})`:`rgba(20,32,24,${alpha})`;ctx.fillRect(x,y,12,4);}

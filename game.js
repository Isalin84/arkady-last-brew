...
function loadLevel(index,restore=false){
 GameAudio.reset();storyHeard=false;storyTimer=0;rescueStage=0;stellaVisible=false;stellaMet=false;$('#radio-message').hidden=true;pathAt=-1;pathField=[];levelIndex=index;const level=LEVELS[index];map=level.map.map(r=>r.split('').map(Number));
...
function start(){
 ...
 if(transition){const minimum=[[0,22,140,100],[0,32,190,140],[0,38,220,165]][levelIndex]||[0,22,140,100];checkpoint={hp:Math.max(75,player.hp),ammo:weapons.map((w,i)=>i===0?Infinity:Math.max(w.ammo,minimum[i]))};loadLevel(levelIndex+1,true);event=['packstart','warestart','maltstart'][levelIndex]||'start';}
...
function interact(){
 ...
 if(rescueStage===3){stellaVisible=true;$('#status').textContent='Стелла освобождена. Подойди к ней у силоса № 4.';toast('ЛЮК ОТКРЫТ · СТЕЛЛА СВОБОДНА');}
...
warehouseStory();
if(levelIndex===3&&stellaVisible&&!stellaMet){const [sx,sy]=LEVELS[3].stella;if(Math.hypot(player.x-sx,player.y-sy)<1.05){stellaMet=true;finish(true);return;}}

...
let map,props=[],levelIndex=0,levelTotal=12,transition=false,checkpoint=null,pathField=[],pathAt=-1,storyHeard=false,storyTimer=0;
...
function loadLevel(index,restore=false){
 GameAudio.reset();storyHeard=false;storyTimer=0;$('#radio-message').hidden=true;pathAt=-1;pathField=[];levelIndex=index;const level=LEVELS[index];map=level.map.map(r=>r.split('').map(Number));
 props=level.props.map(([x,y,type,size,r])=>({x,y,type,size,r}));
...
function start(){
 ...
 if(transition){checkpoint={hp:Math.max(75,player.hp),ammo:weapons.map((w,i)=>i===0?Infinity:Math.max(w.ammo,(levelIndex===1?[0,32,190,140]:[0,22,140,100])[i]))};loadLevel(levelIndex+1,true);event=levelIndex===2?'warestart':'packstart';}
 else if(dead){loadLevel(levelIndex,true);event=['start','packstart','warestart'][levelIndex];}
 else if(!started||won)reset();
 started=true;running=true;$('#overlay').hidden=true;$('#hud').hidden=false;$('#crosshair').style.display='block';$('#status').textContent=levelIndex===2?'Маячок мигает? Уходи с линии тарана!':'Очисти цех и доберись до выхода';
...
function finish(win){
 ...
 GameAudio.say(win?(transition?(levelIndex===0?'transition':'waretransition'):'warefinal'):'death',true);
 running=false;won=win&&!transition;dead=!win;fire=false;keys.clear();document.exitPointerLock?.();$('#overlay').hidden=false;$('#radio-message').hidden=true;
 const next=levelIndex===0?'РОЗЛИВ':'СКЛАД';
 $('.intro h1').innerHTML=transition?'ДАЛЬШЕ —<br><span>'+next+'</span>':win?'СИГНАЛ<br><span>ИЗ СИЛОСА</span>':'СМЕНА<br><span>ПРОПАЛА</span>';
 $('.intro p').innerHTML=transition?(levelIndex===0?'Варочный цех очищен. Но тара ожила!<br>Впереди линии банок и бутылок.':'Розлив спасён. На складе взбесилась техника!<br>Мигающий маячок — предупреждение о таране.<br>Увернись: удар о стеллаж оглушит погрузчик.')+'<br>Припасы пополнены, здоровье — не ниже 75.':win?'Склад очищен. Но в рации — обрывки сигнала о помощи.<br>Монстры завода замуровали коллегу в силосе на солодовне. Связь обрывается. Кто там — пока неизвестно.<br><b>Глава 04: «Солодовая ловушка» — продолжение следует.</b>':`Уничтожено: ${kills} из ${levelTotal}.<br>Попробуй ещё раз с начала этого цеха.`;
 $('#start').innerHTML=(transition?(levelIndex===0?'В цех упаковки':'На склад'):win?'Пройти три главы заново':'Повторить цех')+' <span>↗</span>';
 $('#status').textContent=transition?`Уровень ${levelIndex+1} из ${LEVELS.length} пройден`:win?'Глава 03 завершена · впереди солодовня':'Смена прервана';
}
function shoot(){let w=weapons[weapon];if(cooldown>0)return;if(w.ammo<=0){toast('Припасы закончились. Найди ящик или возьми бутылку: 1');cooldown=.4;return;}w.ammo--;cooldown=w.cool;kick=1;let count=weapon===3?3:1;for(let i=0;i<count;i++){let a=player.a+(i-(count-1)/2)*.1;shots.push({x:player.x+Math.cos(a)*.25,y:player.y+Math.sin(a)*.25,dx:Math.cos(a)*w.speed,dy:Math.sin(a)*w.speed,type:w.type,damage:w.damage,life:weapon===3?.65:2.5,age:0});}GameAudio.shot(w.type);GameAudio.say(['shoot','packshoot','wareshoot'][levelIndex]);updateHUD();}
...
function damage(e,d){if(e.hp<=0)return;e.hp-=d;e.hit=.16;if(e.hp<=0){kills++;burst(e.x,e.y,ENEMY_TYPES[e.type].color,25);GameAudio.kill();GameAudio.say(e.type>=6?'warekill':e.type===5?'golemkill':e.type===3?'packkill':'kill');if(kills===levelTotal)toast('Цех очищен! Выход — в дальнем правом углу. Смотри карту: M');updateHUD();}}
...
function hurtPlayer(amount){
 player.hp=Math.max(0,player.hp-amount);hurt=1;GameAudio.hit();GameAudio.say(player.hp<30?'low':'warehurt');updateHUD();
...
for(let e of enemies){if(e.hp<=0)continue;e.hit=Math.max(0,e.hit-dt);e.attack-=dt;let d=Math.hypot(e.x-player.x,e.y-player.y),visible=sight(e.x,e.y,player.x,player.y);if((d<8&&visible)||e.alert){if(!e.alert){e.alert=true;toast(ENEMY_TYPES[e.type].name+'!');if(e.type<6)GameAudio.monster?.(e.type,d,(-Math.sin(player.a)*(e.x-player.x)+Math.cos(player.a)*(e.y-player.y))/5);}if(e.type>=6){forklift(e,dt,d,visible);if(!running)return;continue;}if(d>.72||!visible){chase(e,dt,d,visible);}else if(e.attack<=0){player.hp=Math.max(0,player.hp-ENEMY_TYPES[e.type].damage);hurt=1;e.attack=e.type===5?1.4:.85;GameAudio.monster?.(e.type,d,0,true);GameAudio.hit();GameAudio.say(player.hp<30?'low':['hurt','packhurt','warehurt'][levelIndex]);updateHUD();if(player.hp<=0){finish(false);return;}}}}
for(let sh of shots){sh.life-=dt;sh.age+=dt;let n=Math.ceil(Math.hypot(sh.dx,sh.dy)*dt/.1);for(let j=0;j<n&&sh.life>0;j++){sh.x+=sh.dx*dt/n;sh.y+=sh.dy*dt/n;if(solid(sh.x,sh.y)){sh.x-=sh.dx*dt/n;sh.y-=sh.dy*dt/n;impact(sh);sh.life=0;break;}let target=enemies.find(e=>e.hp>0&&Math.hypot(e.x-sh.x,e.y-sh.y)<(e.type>=6?.48:.34));if(target){if(sh.type!=='can')damage(target,sh.damage);impact(sh);sh.life=0;}}if(sh.life<=0&&sh.type==='can'&&sh.age>=2.5)impact(sh);}
...
warehouseStory();
if(kills===levelTotal&&Math.hypot(player.x-LEVELS[levelIndex].exit[0],player.y-LEVELS[levelIndex].exit[1])<.85)finish(true);
}
...
studioLogo.onload=()=>{const size=82;const scale=Math.min(size/studioLogo.naturalWidth,size/studioLogo.naturalHeight);const w=studioLogo.naturalWidth*scale*(2*VIEW*Math.tan(FOV/2)/W),h=studioLogo.naturalHeight*scale;for(const wall of [studioWall,studioPackWall,studioWarehouseWall])wall.getContext('2d').drawImage(studioLogo,64-w/2,64-h/2,w,h);};
studioLogo.src='assets/art/brand/logo.png';
function wallTexture(tile,x,y){
 if(tile===4)return SceneArt.rack;
 if(levelIndex===2&&tile===3)return SceneArt.maltDoor;
...
function background(){ctx.fillStyle='#263629';ctx.fillRect(0,0,W,H);let sky=ctx.createLinearGradient(0,0,0,VIEW/2);sky.addColorStop(0,LEVELS[levelIndex].ceiling);sky.addColorStop(1,levelIndex===2?'#61685f':levelIndex?'#72909b':'#68745e');ctx.fillStyle=sky;ctx.fillRect(0,0,W,VIEW/2);let floor=ctx.createLinearGradient(0,VIEW/2,0,VIEW);floor.addColorStop(0,LEVELS[levelIndex].floor);floor.addColorStop(1,'#302f24');ctx.fillStyle=floor;ctx.fillRect(0,VIEW/2,W,H);for(let y=VIEW/2+5;y<VIEW;y+=3){let d=VIEW/(2*(y-VIEW/2));let alpha=Math.min(.17,.8/d);ctx.fillStyle=`rgba(195,171,109,${alpha})`;if(Math.floor(d+player.y)%2===0)ctx.fillRect(0,y,W,1);} }
...
function drawSprites(){let all=[...props.map(p=>({...p,aspect:['bottles','cans'].includes(p.type)?1.7:1,img:SceneArt.prop(p.type,Math.floor(clock*5)%2)})),...enemies.filter(e=>e.hp>0).map(e=>({...e,img:(e.type>=6?SceneArt.forklift(e.type,e.mode,Math.floor(clock*10)%2):e.type>2?SceneArt.monster(e.type,e.attack>.5?1:Math.floor(clock*6+e.seed)%2):sprites['enemy'+e.type]),size:e.type>=6?1.25:e.type===5?1.15:.85,aspect:e.type>=6?1.25:1,z:e.type>=6?0:Math.sin(clock*5+e.seed)*.025})),...items.map(i=>({...i,img:sprites[i.type],size:.52})),...shots.map(s=>({...s,img:sprites[s.type],size:s.type==='foam'?.2:.24,z:.35+Math.sin(Math.min(1,s.age)*Math.PI)*.2}))];all.sort((a,b)=>Math.hypot(b.x-player.x,b.y-player.y)-Math.hypot(a.x-player.x,a.y-player.y));for(let e of all)bill(e.img,e.x,e.y,e.size,e.z||0,e.hit||0,e.aspect||1);for(let q of particles){let p=project(q.x,q.y);if(p.d<.1||p.x<0||p.x>=W||p.d>depth[Math.floor(p.x)])continue;let y=VIEW/2+(.4-q.z)*VIEW/p.d,r=Math.min(20,5/p.d);ctx.fillStyle=q.color;ctx.fillRect(p.x,y,r,r);}}
function gun(){
...
function minimap(){let scale=showMap?15:6,ox=W-16-map[0].length*scale,oy=16;ctx.fillStyle='#10241fe8';ctx.fillRect(ox-7,oy-7,map[0].length*scale+14,map.length*scale+14);map.forEach((row,y)=>row.forEach((t,x)=>{ctx.fillStyle=t===2?'#ac8652':t===3?'#d9d883':t?'#63725b':'#233c2f';ctx.fillRect(ox+x*scale,oy+y*scale,scale-1,scale-1);}));if(showMap){for(let e of enemies){if(e.hp<=0)continue;ctx.fillStyle='#d28966';ctx.fillRect(ox+e.x*scale-2,oy+e.y*scale-2,4,4);}for(let i of items){ctx.fillStyle='#c5dca0';ctx.fillRect(ox+i.x*scale-2,oy+i.y*scale-2,4,4);}}ctx.fillStyle='#ffd778';ctx.beginPath();ctx.arc(ox+player.x*scale,oy+player.y*scale,3,0,7);ctx.fill();ctx.strokeStyle='#ffd778';ctx.beginPath();ctx.moveTo(ox+player.x*scale,oy+player.y*scale);ctx.lineTo(ox+(player.x+Math.cos(player.a)*.7)*scale,oy+(player.y+Math.sin(player.a)*.7)*scale);ctx.stroke();}

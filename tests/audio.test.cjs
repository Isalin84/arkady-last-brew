...
(async()=>{
 const api=vm.runInContext('GameAudio',sandbox);await api.resume();assert.equal(api.loaded.length,37);
 ...
 console.log('PASS: 37 audio assets, voice scheduling, no overlap, pause, mute, variation, walking lines');

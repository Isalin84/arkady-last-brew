...
(async()=>{
 const api=vm.runInContext('GameAudio',sandbox);await api.resume();assert.equal(api.loaded.length,71);
 ...
 api.reset();api.setLevel(3);await api.resume();api.update(14,true,false);assert.ok(/Солод любит|Нория|Запах солода/.test(el.textContent),'Malt house walking dialogue');api.pause();await api.resume();assert.equal(api.say('stellathanks',true),true);assert.ok(el.textContent.startsWith('Стелла:'),'Stella has her own subtitle speaker');
 console.log('PASS: 71 audio assets, four-level dialogue, Stella speaker, scheduling, no overlap, pause, mute, variation');
